...
class Analytics(object):
	def __init__(self, filters=None):
		...
		# Fixe le doc_type à "Sales Order" et le tree_type par défaut si besoin
	...
	def get_data(self):
		# Logique spécifique pour "Par verre"
	...
	# Ajout : méthode pour groupement par secteur
	...
	def get_secteur_groups(self):
		# Récupère tous les secteurs pour la hiérarchie (simple, pas de parent/enfant ici)
	...
	def get_sales_transactions_based_on_glass(self):
		# Détermine le champ à agréger selon le filtre Value/Quantity
		if self.filters["value_quantity"] == "Value":
		...
		self.entries = frappe.db.sql(
			...
			""".format(
				...
				filters=self.get_additional_filters_sql(),
			),
	...
	def get_additional_filters_sql(self):
		# Génère les filtres SQL supplémentaires en fonction des autres filtres
		filters = []
		if self.filters.get("secteur"):
