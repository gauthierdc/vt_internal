...
def _quotation_response(quotation, prompt_name):
    doc = frappe.get_doc("Quotation", quotation, ignore_permissions=True)
    print_html = frappe.get_print("Quotation", doc.name, doc.meta.default_print_format or "Standard", ignore_permissions=True)
