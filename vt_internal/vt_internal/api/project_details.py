...
def project_details():
    ...
    def get_theoretical(project_id, axis):
        ...
        result = frappe.db.sql(sql, project_id, as_dict=1)[0]
        vente = result.vente or 0
        cost = result.cost or 0
        return vente, cost

    def has_pose_vt_items(project_id):
        return bool(frappe.db.sql("""
        SELECT 1 FROM `tabSales Order Item` soi
        INNER JOIN `tabSales Order` so ON so.name = soi.parent
        INNER JOIN `tabItem` i ON i.name = soi.item_code
        WHERE so.project = %s AND so.docstatus = 1 AND i.custom_pose_vt = 1 LIMIT 1
        """, project_id))
    ...
    def get_sales_orders(project_id):
        sos = frappe.db.get_list('Sales Order',
            filters={'project': project_id, 'docstatus': ['!=', 2]},
            fields=["name", "status", "grand_total", "total", "transaction_date", "markup_percentage"]
        )
        items = [{
    ...
    labour_hours = project.custom_estimated_labor_hours or 0
